...
# ====================================================================
# Download all demo scenes to MP4 (native framebuffer capture)
# Run: .venv/bin/python3.12 scenes/demo_scene.py            # all scenes
#      .venv/bin/python3.12 scenes/demo_scene.py 25 1 2     # subset
# ====================================================================
if __name__ == "__main__":
    import os, sys, time, subprocess, tempfile, threading, shutil

    # LaTeX (BasicTeX) for the MathTex scenes
    tex_bin = os.path.expanduser("~/texlive/2026basic/bin/universal-darwin")
    ...
    def _rec_init(self, w=1920, h=1080):
        ...
        out = os.path.join(OUTPUT_DIR, f"{_scene_num:02d}_{safe}.mp4")
        print(f"[{_scene_num:02d}] {_scene_name}  ->  {os.path.basename(out)}")
        self.start_record(out, fps=30)
    ...
    # -- TeX cache helpers (reuse compiled formulas across runs) --
    ...
    only = set(sys.argv[1:])
    if only:
        ALL_SCENES = [s for s in ALL_SCENES if str(s[0]) in only]

    print("=" * 55)
    print("  Real Time Manim - Download All Demo Scenes")
    print(f"  Platform: {sys.platform}   Scenes: {len(ALL_SCENES)}")
    print("=" * 55)
    ok, fail = [], []
    for num, name, cls in ALL_SCENES:
        ...
        try:
            ...
            r = _last_render[0]
            if r is not None and not getattr(r, "_closed", False):
                try:
                    r.close()
                except Exception:
                    pass
    ...
    print()
    print(f"  Done: {len(ok)}/{len(ALL_SCENES)}")
    for n, e in fail:
        print(f"    X {n}: {e[:160]}")
